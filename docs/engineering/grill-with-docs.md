...
The engine is a **grill**: a relentless, one-question-at-a-time walk down the design tree, resolving dependencies between decisions before moving on. Every question lays out the realistic options with their trade-offs — a compact pros/cons table when there are two or more — and offers a recommended answer with the reasoning behind it. Questions the codebase can answer are answered by reading the codebase, not by asking you.
