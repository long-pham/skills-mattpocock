...
It asks **one question at a time** and waits. It never dumps a batch of questions at you — that is bewildering — and where a question can be answered by reading the codebase, it goes and reads rather than asking. Each question lays out the realistic options with their trade-offs (a compact pros/cons table when there are two or more), then names the agent's recommendation and _why_ — so you are weighing a reasoned proposal, not staring at a blank prompt.
